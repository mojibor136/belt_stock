            ...
            <form action="{{ route('stocks.update') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <select id="brand" name="brand"
                        ...
                        @foreach ($brands as $brand)
                            <option value="{{ $brand->id }}"
                                {{ old('brand', $stock->brand_id) == $brand->id ? 'selected' : '' }}>
                                {{ $brand->brand }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="mt-4">
                    ...
                    <select id="group" name="group"
                        class="group w-full border border-gray-300 rounded px-3 py-2 text-md">
                        <option value="{{ $stock->group_id }}">{{ $stock->group->group }}</option>
                    </select>
                </div>
                ...
                <div class="mt-4">
                    ...
                    <select id="size" name="size"
                        class="size w-full border border-gray-300 rounded px-3 py-2 text-md">
                        <option value="{{ $stock->size_id }}">{{ $stock->size->size }}</option>
                    </select>
                </div>

                <div class="mt-4">
                    <label for="quantity" class="block text-md font-medium text-gray-700 mb-1.5">Quantity</label>
                    <input type="text" id="quantity" name="quantity" value="{{ old('quantity', $stock->quantity) }}"
                ...
                <div class="mt-4">
                    <label for="alert" class="block text-md font-medium text-gray-700 mb-1.5">Alert Quantity</label>
                    <input type="text" id="alert" name="alert" value="{{ old('alert', $stock->alert) }}"
                        class="w-full border border-gray-300 rounded-md px-3 py-2 text-md focus:ring-1 focus:ring-blue-600 focus:outline-none text-gray-700">
                </div>
